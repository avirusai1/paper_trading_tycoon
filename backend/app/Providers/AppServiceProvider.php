...
final class AppServiceProvider extends ServiceProvider
{
    public function register(): void
    {
        ...
        $this->app->singleton(XPMultiplierCalculator::class, fn ($app) =>
            new XPMultiplierCalculator($app->make(GameRuleProviderContract::class))
        );
        ...
        $this->app->bind(GrantXPAction::class, fn ($app) =>
            new GrantXPAction(
        ...
        $this->app->bind(GrantMissionProgressAction::class, fn ($app) =>
            new GrantMissionProgressAction(
                $app->make(MissionCriteriaEvaluator::class),
            )
        );

        $this->app->bind(GrantAchievementProgressAction::class, fn ($app) =>
            new GrantAchievementProgressAction(
        ...
        $this->app->bind(GrantSeasonProgressAction::class, fn ($app) =>
            new GrantSeasonProgressAction(
        ...
        $this->app->bind(XPProcessorContract::class, fn ($app) =>
            new XPProcessor(
                $app->make(GrantXPAction::class),
                $app->make(DailyCapTracker::class),
            )
        ...
        $this->app->bind(RewardProcessorContract::class, fn ($app) =>
            new RewardProcessor(
                $app->make(GrantCoinsAction::class),
                $app->make(GameRuleProviderContract::class),
            )
        ...
        $this->app->bind(MissionProcessorContract::class, fn ($app) =>
        ...
        $this->app->bind(CareerProcessorContract::class, fn ($app) =>
            new CareerProcessor($app->make(GrantCareerProgressAction::class))
        );

        $this->app->bind(AchievementProcessorContract::class, fn ($app) =>
            new AchievementProcessor(
        ...
        $this->app->bind(LeagueProcessorContract::class, fn ($app) =>
            new LeagueProcessor($app->make(GrantLeagueProgressAction::class))
        );

        $this->app->bind(SeasonProcessorContract::class, fn ($app) =>
            new SeasonProcessor(
        ...
        $this->app->singleton(GameEventPipeline::class, fn ($app) =>
            new GameEventPipeline(
        ...
        $this->app->singleton(GameEngineContract::class, fn ($app) =>
            new GameEngine(
        ...
        $this->app->singleton(GameEngine::class, fn ($app) =>
            $app->make(GameEngineContract::class)
        );

        // ── Coin Ledger ────────────────────────────────────────────────────────
        $this->app->bind(CoinLedgerContract::class, \App\Services\Economy\CoinLedgerService::class);
        $this->app->bind(\App\Services\Economy\CoinLedgerService::class, \App\Services\Economy\CoinLedgerService::class);
        ...
        $this->app->singleton(MultiplierResolverContract::class, fn ($app) =>
            new MultiplierResolver($app->make(GameRuleProviderContract::class))
        );
        $this->app->singleton(MultiplierResolver::class, fn ($app) =>
            $app->make(MultiplierResolverContract::class)
        );

        // ── Calculators (transient) ───────────────────────────────────────────
        $this->app->bind(XPCalculator::class, fn ($app) =>
            new XPCalculator(
                $app->make(GameRuleProviderContract::class),
                $app->make(MultiplierResolver::class),
            )
        );
        $this->app->bind(CoinCalculator::class, fn ($app) =>
            new CoinCalculator(
                $app->make(GameRuleProviderContract::class),
                $app->make(MultiplierResolver::class),
            )
        );
        $this->app->bind(SeasonBonusCalculator::class, fn ($app) =>
            new SeasonBonusCalculator($app->make(GameRuleProviderContract::class))
        );
        $this->app->bind(ReferralBonusCalculator::class, fn ($app) =>
            new ReferralBonusCalculator($app->make(GameRuleProviderContract::class))
        );
        $this->app->bind(PremiumBonusCalculator::class, fn ($app) =>
            new PremiumBonusCalculator($app->make(GameRuleProviderContract::class))
        );

        // ── Strategies (transient) ────────────────────────────────────────────
        $this->app->bind(XPRewardStrategy::class, fn ($app) =>
            new XPRewardStrategy(
                $app->make(XPCalculator::class),
                $app->make(XPProcessorContract::class),
            )
        );
        $this->app->bind(CoinRewardStrategy::class, fn ($app) =>
            new CoinRewardStrategy(
        ...
        $this->app->singleton(RewardStrategyRegistryContract::class, fn ($app) =>
            new RewardStrategyRegistry([
        ...
        $this->app->singleton(RewardStrategyRegistry::class, fn ($app) =>
            $app->make(RewardStrategyRegistryContract::class)
        );

        // ── Actions (transient) ───────────────────────────────────────────────
        $this->app->bind(DistributeRewardAction::class, fn ($app) =>
            new DistributeRewardAction($app->make(RewardStrategyRegistryContract::class))
        );
        $this->app->bind(RollbackRewardAction::class, fn ($app) =>
            new RollbackRewardAction($app->make(RewardStrategyRegistryContract::class))
        );
        ...
        $this->app->singleton(RewardContextBuilderContract::class, fn ($app) =>
            new RewardContextFactory($app->make(FeatureFlagService::class))
        );
        $this->app->singleton(RewardContextFactory::class, fn ($app) =>
            $app->make(RewardContextBuilderContract::class)
        );

        // ── Reward Pipeline (singleton) ───────────────────────────────────────
        $this->app->singleton(RewardPipeline::class, fn ($app) =>
            new RewardPipeline(
                validators: [
        ...
        $this->app->singleton(RewardEngineContract::class, fn ($app) =>
            new RewardEngine(
                $app->make(RewardContextBuilderContract::class),
        ...
        $this->app->singleton(RewardEngine::class, fn ($app) =>
            $app->make(RewardEngineContract::class)
        );
    }
